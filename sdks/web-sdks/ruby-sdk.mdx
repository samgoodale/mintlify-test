...
You provide credentials for the Ruby SDK using a configuration file, which you can also use to customize the SDK's behavior. You can start with our [sample configuration file](./assets/client configs/client-ruby.yaml). We recommend adding this file to the default path `/etc/kameleoon/client-ruby.yaml`. If you use another location, you must pass the path as an argument to the `Kameleoon::KameleoonClientFactory.Create()` method during initialization. These are the available keys in the latest SDK:
...
```ruby
kameleoon_client.add_data(visitor_code, Kameleoon::ApplicationVersion.new("1.2.3"))
```
