...
export const Context = {
    ...
    DataFile: {
        ...
        Ref: "#datafile",
        Params: {
            FeatureFlags: { Name: "feature_flags", Type: "Dict[str, FeatureFlag]" },
        }
    },
    FeatureFlag: {
        Name: "FeatureFlag",
        ...
        Params: {
            ...
            Variations: { Name: "variations", Type: "Dict[str, Variation]" },
            Rules: { Name: "rules", Type: "List[Rule]" },
            DefaultVariationKey: { Name: "default_variation_key", Type: "str" },
        }
    },
    Rule: {
        Name: "Rule",
...
You should provide credentials for the Python SDK via a configuration file, which you can also use to customize the SDK's behavior. A sample configuration file [can be obtained here](./assets/client configs/client-python.yaml). We suggest installing this file to the default path `/etc/kameleoon/client-python.yaml`, but you can put it in another location and pass the path as an argument to the `KameleoonClient()` constructor method. With the current version of the Python SDK, these are the available keys:
...
```python
from kameleoon.data import ApplicationVersion
...
```python
feature_flags = data_file.feature_flags
```
...
# Retrieve all variations of the feature flag as a dictionary (key = variation key, value = Variation object)
variations = feature_flag.variations

# Retrieve the default variation key of the feature flag
default_variation_key = feature_flag.default_variation_key

# Retrieve all rules of the feature flag
...
```python
# Retrieve all variations of the rule as a dictionary (key = variation key, value = Variation object)
