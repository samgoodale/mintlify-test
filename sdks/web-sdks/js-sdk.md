...
Almost every `KameleoonClient` method may throw an error occassionaly. These errors are deliberately predefined `KameleoonError`s that extend the native JavaScript `Error` class, providing useful messages and special `type` fields with a type `KameleoonException`.
